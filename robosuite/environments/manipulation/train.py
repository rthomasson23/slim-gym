...
class Train(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        self.placement_initializer = UniformRandomSampler(
            name="ObjectSampler",
            mujoco_objects=self.place_object + [self.base_object] + [self.phantom_object],
            x_range=[-0.1, 0.1],
            y_range=[-.2, 0.2],
            rotation=[0, np.pi/4, np.pi/6],
            rotation_axis='z',
        ...
        self.model = ManipulationTask(
            ...
            mujoco_objects=[self.base_object] + self.place_object + [self.phantom_object],
        )
    ...
    def _setup_references(self):
        ...
        self.phantom_object_body_id = self.sim.model.body_name2id(self.phantom_object.root_body)
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            for obj_pos, obj_quat, obj in object_placements.values():
                if obj == self.phantom_object:
                    self.goal_pos = np.array(obj_pos)
                    self.goal_pos[2] += 0.2
                    self.sim.data.set_joint_qpos(obj.joints[0], np.concatenate([np.array([100,100,100]), np.array(obj_quat)]))
                else:
